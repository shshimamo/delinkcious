...
import (
	...
	"github.com/shshimamo/delinkcious/pkg/user_manager"
)
...
func Run() {
	...
	store, err := user_manager.NewDbUserStore(dbHost, dbPort, "postgres", "postgres")
	if err != nil {
		log.Fatal(err)
	}
	...
	svc, err := user_manager.NewUserManager(store)
	if err != nil {
		log.Fatal(err)
	}
